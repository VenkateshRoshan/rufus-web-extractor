...
class Crawler:
    ...
    def __init__(
        ...
        logger.info(f"Crawler initialized with max_depth={max_depth}, max_pages={max_pages}")
    ...
    def _extract_content(self, soup: BeautifulSoup, url: str) -> Dict[str, Any]:
        ...
        content = ""
        main_content_tags = ['article', 'main', 'div[role="main"]', '.content', '#content']
    ...
    def _check_page_relevance(self, content: Dict[str, Any], main_content: str) -> bool:
        ...
        try:
            ...
            is_relevant = RelevanceChecker.check_relevance(main_content=main_content, check_content=text_to_check)
            
            if is_relevant:
                logger.info(f"Page {content['url']} is relevant to the main content")
            else:
                logger.info(f"Page {content['url']} is not relevant to the main content")
                
            return is_relevant
            
        except Exception as e:
            logger.error(f"Error checking relevance for {content['url']}: {str(e)}", exc_info=True)
    ...
    def _fetch_page(self, url: str) -> Tuple[Optional[str], Optional[BeautifulSoup]]:
        ...
        """
        try:
            print(f"\rProcessed URLs: {len(self.visited_urls)}/{self.max_pages} | Current: {url}", end="", flush=True)
    ...
    def crawl(self, start_url: str, instructions: str = "") -> Dict[str, Any]:
        ...
        """
        logger.info(f"Starting crawl from {start_url} with instructions: {instructions}")
        ...
        if not start_soup:
            ...
            return {"relevant_pages": {}, "all_visited_urls": list(self.visited_urls), "queue_links": queue_links}
        ...
        while url_queue and pages_processed < self.max_pages:
            ...
            logger.info(f"Processed page {pages_processed}/{self.max_pages}: {normalized_url}")
        ...
        logger.info(f"Crawl completed. Visited {len(self.visited_urls)} URLs, extracted content from {len(self.extracted_content)} pages")
        ...
        print(f"Returning crawl results with {len(self.extracted_content)} relevant pages")
    ...
    def crawl_parallel(self, start_url: str, instructions: str = "") -> Dict[str, Any]:
        ...
        def process_url():
            ...
            while not url_queue.empty() and pages_processed < self.max_pages and not stop_processing:
                try:
                    # Get the next URL and its depth
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            try:
                ...
                print(f"Queue join completed or timed out: {str(e) if not isinstance(e, TimeoutError) else 'Timeout'}")
            
            # Cancel any remaining tasks
            for worker in workers:
                worker.cancel()
        
        logger.info(f"Parallel crawl completed. Visited {len(self.visited_urls)} URLs, extracted content from {len(self.extracted_content)} pages")
        print(f"Saving parallel crawl results with {len(self.extracted_content)} relevant pages")
...
def crawl_website_parallel(url: str, instructions: str = "", **kwargs) -> Dict[str, Any]:
    """
    ...
    Returns:
        Dict[str, Any]: Dictionary containing relevant_pages, all_visited_urls, and queue_links.
    """
    logger.info(f"Starting parallel website crawl for {url} with instructions: {instructions}")
    crawler = Crawler(**kwargs)
...
if __name__ == "__main__":
    ...
    test_instructions = "Information about Python programming language, features, and documentation"

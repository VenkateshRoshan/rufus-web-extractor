...
def extract_main_content(soup: BeautifulSoup) -> str:
    ...
    for noise in soup.select('script, style, header, footer, nav, .nav, .navigation, .header, .footer, .sidebar, .menu, .comments, .ad, .advertisement, aside, form'):
        noise.extract()
    
    # Try to find main content by common container patterns
    content_containers = [
        'article', 'main', 'div[role="main"]', '.content', '#content', '.post', '.entry',
        '.post-content', '.article-content', '.entry-content', '.story', '.post-body'
    ]
...
def extract_links_with_context(soup: BeautifulSoup, base_url: str) -> List[Dict[str, str]]:
...
def extract_images(soup: BeautifulSoup) -> List[Dict[str, str]]:
    ...
    for img in soup.find_all('img'):
        ...
        if not caption:
            ...
            caption_elements = img.find_next_siblings(class_=re.compile('caption|wp-caption-text'))
            if caption_elements:
...
def search_in_content(content: Dict[str, Any], query: str) -> List[Dict[str, Any]]:
    ...
    if 'headings' in content and content['headings']:
        ...
        if score > 0:
            matching_headings = [h for h in content['headings'] 

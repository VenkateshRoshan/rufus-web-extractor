...
class VectorDBCreator:
    def __init__(self, config_path: str = "config/config.yaml"):
        ...
        try:
            ...
            self.embedding_function = embedding_functions.SentenceTransformerEmbeddingFunction(
                model_name=config.get('vector_store.embedding_model', "all-MiniLM-L6-v2")
            )
    ...
    def create_collection(self, collection_name: str) -> chromadb.Collection:
        ...
        try:
            ...
            logger.error(f"Error creating/getting collection: {collection_name}", exc_info=True)
            raise
    ...
    def process_documents(
        ...
        try:
            ...
            logger.info(f"Successfully processed all documents for collection: {collection_name}")
            
        except Exception as e:
            logger.error(f"Error processing documents for collection: {collection_name}", exc_info=True)
            raise

...
class RAGHandler:
    ...
    def __init__(
        self,
        embedding_model: str = config.get('vector_store.embedding_model', "all-MiniLM-L6-v2"),
    ...
    def setup_retrieval_qa(
        ...
        try:
            ...
            logger.info(f"RetrievalQA chain setup complete for collection: {collection_name} with model: {self.model_type}")
            return qa_chain
            
        except Exception as e:
            logger.error(f"Failed to setup RetrievalQA for collection: {collection_name}", 
                        exc_info=True)
            raise
    ...
    def generate_answer(
        ...
        try:
            ...
            logger.info(f"Generated answer for query using {self.model_type} model: {query[:50]}...")
            return result
    ...
    def batch_generate_answers(
        ...
        try:
            ...
            logger.info(f"Batch processed {len(queries)} queries using {self.model_type} model")
            return results
